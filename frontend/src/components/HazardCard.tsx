 ...
 * Shows icon, level badge, score bar, and key details.
 */
...
const HAZARD_CONFIG = {
  ...
  heat: {
    ...
    detailLabel: "Avg Max Temp (°C)",
  },
  storm: {
    label: "Storm & Wind",
    icon: "🌀",
    detailKey: "avg_wind_speed_ms",
    detailLabel: "Avg Wind (m/s)",
  },
};
...
export default function HazardCard({ type, score }: HazardCardProps) {
  ...
  const detailDisplay = detailValue !== undefined
    ? String(detailValue)
    : "N/A";
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span
          className={`text-xs font-bold px-2 py-1 rounded-full border ${badgeClass}`}
        >
          {score.level}
        </span>
      </div>
